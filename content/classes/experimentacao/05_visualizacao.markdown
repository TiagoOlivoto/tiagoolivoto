...
<div class="figure">
<img src="/classes/experimentacao/05_visualizacao_files/figure-html/unnamed-chunk-6-1.png" alt="Um panel para cada nível da variável DAP" width="960" />
<p class="caption">Figure 2: Um panel para cada nível da variável DAP</p>
</div>
...
arrange_ggplot(s1, s2,
               tag_levels = list(c("s1", "s2")),
               widths = c(1, 1.2))
```

<div class="figure" style="text-align: center">
<img src="/classes/experimentacao/05_visualizacao_files/figure-html/unnamed-chunk-9-1.png" alt="Gráfico de dispersão, combinando pontos e linhas de regressão." width="960" />
...
<div class="figure" style="text-align: center">
<img src="/classes/experimentacao/05_visualizacao_files/figure-html/unnamed-chunk-10-1.png" alt="Gráfico do tipo boxplot combinando mapeamentos estéticos." width="864" />
...
h1 <- ggplot(df_maize, aes(x = APLA)) +
  geom_histogram()

h2 <- ggplot(df_maize, aes(x = APLA)) +
...
<div class="figure" style="text-align: center">
<img src="/classes/experimentacao/05_visualizacao_files/figure-html/unnamed-chunk-11-1.png" alt="Gráfico do tipo histograma com estimativas de função de probabilidade kernel e normal." width="672" />
<p class="caption">Figure 6: Gráfico do tipo histograma com estimativas de função de probabilidade kernel e normal.</p>
</div>

No histograma (h), a linha vermelha representa a estimativa da função de probabilidade normal. Para isto, a escala do eixo `y` foi mudada de contagem para densidade.
...
<div class="figure" style="text-align: center">
<img src="/classes/experimentacao/05_visualizacao_files/figure-html/unnamed-chunk-12-1.png" alt="Gráfico do tipo barras, com mapeamento estético e barras de erro." width="960" />
<p class="caption">Figure 7: Gráfico do tipo barras, com mapeamento estético e barras de erro.</p>
</div>

A afirmação de que um gráfico `ggplot2` é feito em camadas fica mais evidente aqui. No gráfico p1, as barras representam as médias geral dos híbridos em nosso conjunto de dados. No segundo gráfico, um novo argumento visto (`fill = SOM`). Isto informa que as barras devem ser coloridas para cada nível do fator `SOM`. A função `stat_summary()`, também vista pela primeira vez aqui, foi utilizada no segundo gráfico para substituir a função `geom_bar()`. Com isto, foi possível incluir as médias (`fun = mean` e  `geom = "bar`), bem como as barras de erro (`fun.data = mean_se` e `geom = "errorbar"`).

Utilizando a função `plot_factbars()` do pacote metan, um gráfico semelhante pode ser criado com

```r
metan1 <- plot_bars(df, DAP, MST)
metan2 <- plot_factbars(df, DAP, SOM, resp = MST)
...
<img src="/classes/experimentacao/05_visualizacao_files/figure-html/unnamed-chunk-13-1.png" width="672" />
...
arrange_ggplot(l1, l2, tag_levels = list(c("l1", "l2")))
```

<div class="figure" style="text-align: center">
<img src="/classes/experimentacao/05_visualizacao_files/figure-html/unnamed-chunk-14-1.png" alt="Gráfico de dispersão combinado com inclusão de curvas ajustadas." width="672" />
<p class="caption">Figure 8: Gráfico de dispersão combinado com inclusão de curvas ajustadas.</p>
</div>
...
<img src="/classes/experimentacao/05_visualizacao_files/figure-html/unnamed-chunk-15-1.png" width="672" />


# Referências

[^1]: WICKHAM, H. **Ggplot2 : elegant graphics for data analysis**. [S.l.]: Springer, 2009.
[^2]: WILKINSON, L. **The grammar of graphics**. [S.l.]: Springer, 2005.
